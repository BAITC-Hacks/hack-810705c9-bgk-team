...
## Обновление интеграции с PR #12 (2026-09-23)

В ветку PR #15 (`feat/adr-009-bff-api-contract`) переносится PR #12 на commit
`3b0d027`, уже содержащий Task Match из #16 и актуальный frontend main.
Канонические данные теперь принадлежат Drizzle/`nextjs_db`, а score/fit,
права, прожарка, отклики и этапы — интегрированным use-case ADR-004–008.
Prototype Map/globalThis runtime, actor-заглушка, дубли score/fit и альтернативный
AI-порт не должны становиться рабочим backend после слияния.

Общие контракты и handlers ADR-009 согласуются с действующими UI/BFF DTO.
Сохраняются русские ошибки `{ error: { code, message, details? } }`, ошибка
входной схемы 422, ошибка серверной схемы ответа 500 и проверки роли/владельца.
Stateless `analyze-text` / `phrase-question` уже приходят из адаптированного
PR #12; Mastra не становится владельцем бизнес-состояния или рейтинга.

**Следующие разделы описывают первоначальный прототип PR #15.** Его 22 теста,
typecheck и lint выполнялись до интеграции и не доказывают работу текущего
Drizzle backend, live Mastra или итогового merge. По просьбе пользователя новые
тесты не запускаются. Детали текущего решения и ограничения записаны в
[заметке интеграции PR #15](../2026-09-23-pr15-task-match-integration.md).

## Историческая реализация до интеграции
