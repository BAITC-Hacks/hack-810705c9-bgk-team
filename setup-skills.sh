#!/usr/bin/env bash
# Default: offline team guide + full library. --extras: fetch additional skills.
# Usage: ./setup-skills.sh [target-dir] [--extras] [max-retries] [retry-delay-sec]
...
TARGET_DIR="${1:-.}"
INSTALL_EXTRAS="${2:-}"
if [ -n "$INSTALL_EXTRAS" ] && [ "$INSTALL_EXTRAS" != "--extras" ]; then
  echo 'Usage: ./setup-skills.sh [target-dir] [--extras] [max-retries] [retry-delay-sec]' >&2
  exit 1
fi
MAX_RETRIES="${3:-3}"
RETRY_DELAY="${4:-2}"
...
# Локальная библиотека cc-polymath: 26 скиллов и все справочные материалы.
bash "$SCRIPT_DIR/setup-polymath.sh" "$TARGET_DIR" || exit 1
if [ "$INSTALL_EXTRAS" != "--extras" ]; then
  echo 'Готово без сети. Дополнительные скиллы: ./setup-skills.sh <project> --extras'
  exit 0
fi
