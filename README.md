...
> **Демо-режим, без аутентификации** ([ADR-008](docs/adr/008-demo-roles-and-access.md)).
> Роль и участник выбираются переключателем и хранятся в cookie `tm_role` / `tm_actor`,
> которые можно подделать. Развёртывание за пределами локального демо не допускается
> без замены этого механизма на настоящую аутентификацию.
